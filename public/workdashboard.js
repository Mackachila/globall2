    ...
    .then(data => {
      ...
      if (team && team.length > 0) {
       ...
       // document.getElementById('today').textContent = totals.todayProfit.toFixed(2);
        document.getElementById('yesterday').textContent = totals.profit3Percent.toFixed(2);
      } else {
        // No transactions, hide the table and display a message
...
document.addEventListener('DOMContentLoaded', function () {
  setTimeout(() => {
    ...
    const todayProfit = parseFloat(todayProfitElement.textContent.replace('KES ', ''));
    const yesterdayProfit = parseFloat(yesterdayElement.textContent.replace('KES. ', ''));

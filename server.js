...
app.post('/register', express.urlencoded({ extended: true }), async (req, res) => {
  ...
  //const updateAdvertsQuery = `
   // INSERT INTO adverts (username, todays_total, status)
    //  VALUES (?, DEFAULT, DEFAULT)
 // `;
 // await pool.promise().execute(updateAdvertsQuery, [username]);
...
// Call the function to get the Nairobi date
...
app.post('/verifyPayment', (req, res) => {
  ...
  pool.query('SELECT * FROM deposits WHERE transactionid = ?', [transactionId], (err, results) => {
    if (err) {
      ...
      res.status(400).json({
        error: 'No payment found with that ID. If you just made a deposit now, please try again after 5 minutes or check the code and try again'
      });
    } else {
...
app.post('/update-level/:level', isAuthenticated, async (req, res) => {
  ...
  try {
    ...
    const [result] = await pool.promise().execute(updateQuery, [calculateTodaysTotal(level, ballance), username]);
